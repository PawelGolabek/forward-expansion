...
function onEnter(){
    ...
    for (var i = 0; i < totalUnits; i++) {
		...
		spread = 0;
		a = instance_create_depth(x - spread + random(spread*2),y - (spread * 0.6) + random(spread * 2 * 0.6), 0, o_emp_explosion);
		instance_destroy()
}
...
function onDragging(){
	...
	with(o_unit){
		if(antiAir > 0 and point_distance_ellipse_sq(x, y, myX, myY, 0.6) <= myRange * myRange){
			greenGlow = true;	
			wantCircle = true;		
		}
	}
